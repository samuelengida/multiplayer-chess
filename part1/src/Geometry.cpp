...
void Geometry::Gen(std::vector<float>& bufferData, std::vector<unsigned int>& indices){
	m_vertexPositions = bufferData;
	m_indices = indices;

	// Populate m_tangents and m_bitangents
	for (unsigned int i = 0; i < indices.size() / 3; i++){
		generateTangent(indices[i*3 + 0], indices[i*3 + 1], indices[i*3 + 2]);
	}

	for(int i =0; i < bufferData.size()/8; ++i){
		// vertices
		m_bufferData.push_back(bufferData[i*8+ 0]);
		m_bufferData.push_back(bufferData[i*8+ 1]);
		m_bufferData.push_back(bufferData[i*8+ 2]);
		// m_normals
		m_bufferData.push_back(bufferData[i*8+3]);
		m_bufferData.push_back(bufferData[i*8+4]);
		m_bufferData.push_back(bufferData[i*8+5]);
    	// texture information
		m_bufferData.push_back(bufferData[i*8+6]); 
		m_bufferData.push_back(bufferData[i*8+7]); 
		// tangents
...
// generate tangent and bitangent values
void Geometry::generateTangent(unsigned int vert0, unsigned int vert1, unsigned int vert2) {

	// m_vertexPositions has all vertices, textures and normal data.
	...
	glm::vec2 tex0(m_vertexPositions[vert0*2 +6], m_vertexPositions[vert0*2 + 7]); 
	glm::vec2 tex1(m_vertexPositions[vert1*2 +6], m_vertexPositions[vert1*2 + 7]); 
	glm::vec2 tex2(m_vertexPositions[vert2*2 +6], m_vertexPositions[vert2*2 + 7]); 
	...
	// Compute a tangent
	m_tangents.push_back(tangent.x); m_tangents.push_back(tangent.y); m_tangents.push_back(tangent.z);
	m_tangents.push_back(tangent.x); m_tangents.push_back(tangent.y); m_tangents.push_back(tangent.z);	
	m_tangents.push_back(tangent.x); m_tangents.push_back(tangent.y); m_tangents.push_back(tangent.z);

	// Compute a bi-tangent
	m_biTangents.push_back(bitangent.x); m_biTangents.push_back(bitangent.y); m_biTangents.push_back(bitangent.z);
	m_biTangents.push_back(bitangent.x); m_biTangents.push_back(bitangent.y); m_biTangents.push_back(bitangent.z);	
	m_biTangents.push_back(bitangent.x); m_biTangents.push_back(bitangent.y); m_biTangents.push_back(bitangent.z);
}
